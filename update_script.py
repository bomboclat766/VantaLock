...
# Locate main script tag near the end of body
script_pattern = re.compile(r'<script>\s*/\* ===== Lenis Smooth Scroll ===== \*/.*?</script>', re.DOTALL)
...
new_script = """<script>
    ...
    function splitToWords(element) {
      ...
      text.split(' ').forEach((word, idx) => {
        const span = document.createElement('span');
        span.className = 'word';
        span.textContent = word + (idx < text.split(' ').length - 1 ? '\u00A0' : '');
        element.appendChild(span);
        words.push(span);
      });
      return words;
...
# Replace in html
html_new = script_pattern.sub(new_script, html)

with open('landing-page/index.html', 'w', encoding='utf-8') as f:
    f.write(html_new)

print("Landing page script updated successfully!")
